...
const float kh = 0.00005037117;
const float kw = 0.00003777838;
...
{
    float y = 0.0008 / (kh * (blob->height));
    float x = (0.06 * ((blob->corner_x) + (blob->width) / 2)) / (blob->height);
    float distance = sqrt(((y * 0.1511352 - x) * (y * 0.1511352 - x)) + y * y);
    return distance;
}

float find_angle_to_blob(Blob *blob)
{
    float a = 40 - ((blob->corner_x) + (blob->width) / 2);
    float angle = atan(kw * a * 100);
    return angle;
}

void map_blob(Blob *blob, Map_blob *blob_pos, float *uv_velocity,float* position)
{
    float distance = find_distance_to_blob(blob) + 0.03125;
    float angle = find_angle_to_blob(blob);
    float vector[2];
    rotate_vector(uv_velocity, vector, angle);
    blob_pos->x = distance * vector[0] + position[0];
    blob_pos->y = distance * vector[1] + position[1];
}